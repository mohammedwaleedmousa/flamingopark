...
const FRESH_RELOAD_PARAM = "__fp_reload";

const isChunkLoadError = (error: unknown) => {
  const message = String((error as { message?: unknown })?.message || error || "");
  return /Failed to fetch dynamically imported module|Expected a JavaScript-or-Wasm module script|Importing a module script failed|ChunkLoadError|Loading chunk|Load failed|MIME type of "text\/html"/i.test(message);
};

const forceFreshReload = () => {
  const url = new URL(window.location.href);
  url.searchParams.set(FRESH_RELOAD_PARAM, Date.now().toString());
  window.sessionStorage.removeItem(`flamingo-lazy-retry:${window.location.pathname}`);
