              ...
              {slides.map((slide, index) => (
                <SwiperSlide key={`${slide.image}-${index}`}>
                  <div className="relative h-[230px] w-full overflow-hidden bg-muted/30 sm:h-[285px] md:h-[500px] lg:h-[560px] xl:h-[620px] 2xl:h-[660px]">
                    <img src={optimizeImage(slide.image, heroImageWidth, index === 0 ? 78 : 72)} alt={slide.title || "Flamingo Park"} loading="eager" decoding="async" fetchPriority={index === 0 ? "high" : "auto"} width={heroImageWidth} height={1000} onError={handleImageError} className="absolute inset-0 h-full w-full object-cover object-center" style={{ objectPosition: `${slide.imagePositionX}% ${slide.imagePositionY}%`, transform: `scale(${slide.imageZoom})` }} />
