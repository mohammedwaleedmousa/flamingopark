...
const HomeManagedSections = () => {
  ...
  return <>{rendered.map(({ section, products }) => products.length > 0 ? <div key={section.id}><section className="bg-background py-7 md:py-12"><div className="mx-auto w-full max-w-[1400px] px-3 md:px-6"><div className="mb-4 md:mb-7"><div className="min-w-0"><div className="mb-1 flex items-center gap-2"><span className="h-[2px] w-4 shrink-0 rounded-full bg-[#D4777D]" /><span className="truncate font-serif text-[6px] uppercase tracking-[0.2em] text-[#B86168] md:text-[7px]">{section.title || "FLAMINGO EDIT"}</span></div><h2 className="text-[17px] font-semibold tracking-[-0.025em] text-foreground md:text-[26px]">{section.title_ar}</h2></div></div><div className="grid grid-cols-2 gap-x-2.5 gap-y-5 sm:gap-x-3 md:grid-cols-4 md:gap-x-5 md:gap-y-7">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div>{section.show_view_all !== false && <div className="mt-7 flex justify-center md:mt-9"><Link to={section.view_all_link || "/products"} className="inline-flex h-10 items-center justify-center gap-1.5 rounded-full border border-[#D8B5B2] bg-white px-6 text-[8px] font-semibold text-[#A95B61] transition-colors hover:bg-[#FFF7F5] md:h-11 md:px-8 md:text-[9px]">عرض المزيد<ArrowLeft className="h-3.5 w-3.5" strokeWidth={1.5} /></Link></div>}</div></section>{section.filter_type === "featured" ? <MidPageBanner /> : null}</div> : null)}</>;
};
