...
const ColorVariantsEditor = ({ value, onChange }: Props) => {
  ...
  useEffect(() => {
    ...
    const blockSubmitWhileUploading = (event: Event) => {
      ...
      toast({
        title: 'الصور ما زالت تُرفع',
        description: 'انتظر اكتمال رفع الصور ثم احفظ المنتج.',
      });
    };
    ...
    return () => {
      form.removeEventListener('submit', blockSubmitWhileUploading, true);
    };
  }, [uploading]);
  ...
  const uploadImage = async (colorIdx: number, files: FileList) => {
    ...
    try {
      ...
      if (failedFiles.length > 0) {
        toast({
          title: `تم رفع ${successfulUrls.length} صور`,
          description: `تعذر رفع ${failedFiles.length} صور. يمكنك إعادة المحاولة.`,
        });
      } else {
        toast({
          title: successfulUrls.length === 1
            ? 'تم رفع الصورة بنجاح'
            : `تم رفع ${successfulUrls.length} صور بنجاح`,
          description: 'تم حفظ الصور على Supabase.',
        });
      }
    } catch (error: any) {
  ...
  const removeImage = (colorIdx: number, imgIdx: number) => {
    ...
    next[colorIdx] = {
      ...next[colorIdx],
      images: next[colorIdx].images.filter(
        (_, i) => i !== imgIdx,
      ),
    };
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (files.length > remaining) {
                          toast({
                            title: `يمكنك إضافة ${remaining} صور فقط`,
                            variant: 'destructive',
                          });
