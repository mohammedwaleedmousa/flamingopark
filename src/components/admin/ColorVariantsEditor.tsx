...
const IMAGE_PREPARE_TIMEOUT_MS = 45_000;
...
const validateImage = (file: File) => {
  ...
  if (!ALLOWED_EXTENSIONS.includes(extension) && !ALLOWED_IMAGE_TYPES.includes(type)) {
...
const uploadProductImage = async (file: File): Promise<string> => {
  ...
  let finalFile: File;
  try {
    finalFile = await withTimeout(
      prepareImageUpload(file, PRODUCT_IMAGE_OPTIONS),
      IMAGE_PREPARE_TIMEOUT_MS,
      `استغرقت معالجة ${file.name} وقتاً طويلاً. جرّب الصورة مرة أخرى`,
    );
  } catch (error: any) {
  ...
  const { error } = await withTimeout(
    supabase.storage.from('uploads').upload(fileName, finalFile, {
      cacheControl: '31536000',
      upsert: false,
      contentType: 'image/webp',
    }),
    IMAGE_UPLOAD_TIMEOUT_MS,
    `انتهت مهلة رفع ${file.name}. تحقق من الإنترنت وحاول مرة أخرى`,
  );

  if (error) throw error;

  const { data } = supabase.storage.from('uploads').getPublicUrl(fileName);
  if (!data.publicUrl) throw new Error(`تعذر إنشاء رابط صورة ${file.name}`);
...
const ColorVariantsEditor = ({ value, onChange }: Props) => {
  ...
  const uploadImage = async (colorIdx: number, files: FileList) => {
    ...
    const originalImages = [...(value[colorIdx].images || [])];

    if (originalImages.length + fileArray.length > MAX_IMAGES_PER_COLOR) {
      toast({
    ...
    try {
      fileArray.forEach(validateImage);
    } catch (error: any) {
      toast({
        title: 'الصورة غير صالحة',
        description: error?.message || 'اختر صورة بصيغة مدعومة',
        variant: 'destructive',
      });
      return;
    }
    ...
    optimistic[colorIdx] = {
      ...optimistic[colorIdx],
      images: [...originalImages, ...previewUrls],
    };
    onChange(optimistic);

    try {
      const results = await Promise.allSettled(fileArray.map(uploadProductImage));
      ...
      if (uploadedUrls.length === 0) {
        ...
        throw firstError instanceof Error ? firstError : new Error('تعذر رفع الصور');
      }

      const next = [...value];
      next[colorIdx] = {
        ...next[colorIdx],
        images: [...originalImages, ...uploadedUrls],
      };
      onChange(next);

      if (failedResults.length > 0) {
        toast({
          title: `تم رفع ${uploadedUrls.length} صور`,
          description: `تعذر رفع ${failedResults.length} صور. يمكنك إعادة المحاولة للصور التي فشلت.`,
        });
      } else {
        toast({
          title: `تم رفع ${uploadedUrls.length} صور`,
      ...
      toast({
        title: 'فشل رفع الصور',
        description: error?.message || 'حدث خطأ أثناء رفع الصور إلى Supabase',
        variant: 'destructive',
      });
    } finally {
              ...
              <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                ...
                <label
                  className={`aspect-square border-2 border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition ${
                    uploading === ci ? 'opacity-60 cursor-not-allowed' : 'hover:border-primary'
                  }`}
                >
                  {uploading === ci ? (
                    <>
                    ...
                    <>
                      ...
                      <span className="text-[11px] mt-2 text-center">اختر حتى 5 صور</span>
                      <span className="text-[10px] text-muted-foreground">يمكن اختيار عدة صور دفعة واحدة</span>
                    </>
                  )}
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (files.length > MAX_IMAGES_PER_COLOR) {
                        toast({
                      ...
                      <div key={si} className="flex items-center gap-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            sizes[si] = {
                              ...variantSize,
                              stock: Math.max(0, parseInt(e.target.value || '0') || 0),
                            };
                            next[ci].sizes = sizes;
                            onChange(next);
                          }}
                ...
                {(c.sizes || []).length === 0 && (
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-[11px] text-muted-foreground">
                      استخدمها للحقائب والساعات والمنتجات بلا مقاسات
                    </span>
                  </div>
                )}
                  ...
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        if (!raw) return;
                        const next = [...value];
                        const sizes = (next[ci].sizes || []).map((size) =>
