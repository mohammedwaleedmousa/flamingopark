...
const syncAvatarImages = (url = "") => {
  document.querySelectorAll<HTMLImageElement>("img[alt='معاينة الصورة']").forEach((img) => {
    if (url) img.src = url;
    else img.replaceWith(Object.assign(document.createElement("span"), { textContent: "" }));
  });
};

const closeSheet = () => { actionSheet?.remove(); actionSheet = null; };
...
const showAvatarActions = (form: HTMLFormElement, fileInput: HTMLInputElement) => {
  ...
  actionSheet = sheet;
  sheet.dir = "rtl";
  sheet.style.cssText = "position:fixed;inset:0;z-index:10050;background:rgba(28,23,21,.35);display:flex;align-items:flex-end;justify-content:center;padding:12px 12px calc(env(safe-area-inset-bottom) + 12px);";
  sheet.innerHTML = `<div style="width:100%;max-width:440px;display:grid;gap:8px">
  ...
  sheet.querySelector("[data-delete]")?.addEventListener("click", async () => {
    ...
    if (error) { if (button) { button.disabled = false; button.textContent = "تعذر الحذف - حاول مجددًا"; } return; }
...
const enhanceEditSheet = () => {
  ...
  document.querySelectorAll<HTMLFormElement>("form").forEach((form) => {
    ...
    form.style.maxHeight = "calc(100dvh - 150px)";
