...
const openCropper = (file: File, input: HTMLInputElement) => {
  ...
  const render = () => {
    clamp();
    img.style.width = `${img.naturalWidth * baseScale * zoom}px`;
    img.style.height = `${img.naturalHeight * baseScale * zoom}px`;
    img.style.transform = `translate(calc(-50% + ${offsetX}px), calc(-50% + ${offsetY}px))`;
  };
  img.onload = () => { baseScale = Math.max(240 / Math.max(1, img.naturalWidth), 240 / Math.max(1, img.naturalHeight)); render(); };
  ...
  overlay.querySelector("[data-confirm]")?.addEventListener("click", async () => {
    ...
    const holder = preview ? null : form?.querySelector<HTMLElement>(".h-\\[82px\\].w-\\[82px\\]");
...
export const installAccountProfileRuntimeFix = () => {
  ...
  document.addEventListener("change", (event) => {
    ...
    if (!(input instanceof HTMLInputElement) || input.type !== "file" || !isAccount()) return;
    const form = input.closest("form");
    ...
    if (!file.type.startsWith("image/") && !/\.(heic|heif)$/i.test(file.name)) { addMessage(form, "اختر صورة صحيحة"); return; }
  ...
  document.addEventListener("submit", async (event) => {
    ...
    if (!(form instanceof HTMLFormElement) || !isAccount() || !pendingAvatar || !form.textContent?.includes("حفظ التغييرات")) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    ...
    try {
      ...
      const name = form.querySelector<HTMLInputElement>("input[type='text']")?.value.trim() || stored.name || "";
      const regionText = Array.from(form.querySelectorAll("button")).find((button) => button.textContent?.includes("المحافظة"))?.textContent || "";
      const region = YEMEN_REGIONS.find((item) => regionText.includes(item)) || stored.region || stored.country || "عدن";
      const { data, error } = await (supabase as any).rpc("customer_update_self", { _id: stored.id, _phone: stored.phone || "", _name: name, _region: region, _avatar_url: avatarUrl });
...
installAccountProfileRuntimeFix();
