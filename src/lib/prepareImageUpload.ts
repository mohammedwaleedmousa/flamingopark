...
export async function prepareImageUpload(
  ...
  const looksLikeHeicByNameOrType = type === "image/heic" || type === "image/heif" || name.endsWith(".heic") || name.endsWith(".heif");
...
export async function uploadPreparedImage(prepared: File, pathPrefix: string): Promise<string> {
  ...
  if (sessionError || !session?.access_token) throw new Error("انتهت جلسة الأدمن. سجّل الدخول ثم حاول مرة أخرى");
