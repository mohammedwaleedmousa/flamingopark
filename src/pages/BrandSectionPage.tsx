...
const BrandSectionPage = () => {
  ...
  const { data: brand, isLoading: brandLoading } = useQuery({
    queryKey: ["brand-by-slug-sec-v3", slug],
  ...
  const { data: section, isLoading: sectionLoading } = useQuery({
    queryKey: ["brand-section-v3", brand?.id, sectionSlug],
  ...
  const { data: products = [], isLoading: productsLoading, isError: productsError } = useQuery({
    queryKey: ["brand-section-products-v3", section?.id, brand?.id],
    ...
    queryFn: async () => {
      ...
      const productIds = Array.from(
        new Set((relations || []).map((row: any) => row.product_id).filter(Boolean)),
      ) as string[];

      if (!productIds.length) return [] as Product[];
      ...
      if (error) throw error;
      return (rows || []).map(mapProductCard) as Product[];
    },
    staleTime: 30_000,
    refetchOnMount: "always",
    refetchOnWindowFocus: false,
      ...
      <div className="flex min-h-screen flex-col bg-background" dir="rtl">
        ...
        <main className="flex flex-1 items-center justify-center px-4">
          <div className="text-center">
            ...
            <Link to={`/brands/${slug}`} className="mt-4 inline-flex items-center gap-1 text-sm text-[#A95B61]">
              <ChevronRight className="h-4 w-4" /> العودة إلى الماركة
            </Link>
          </div>
        </main>
        <Footer />
    ...
    <div className="flex min-h-screen flex-col bg-background" dir="rtl">
      ...
      <main className="flex-1 pb-14">
        <section className="mx-auto w-full max-w-[1400px] px-4 pb-5 pt-6 md:px-6 md:pb-7 md:pt-8">
          <Link to={`/brands/${slug}`} className="inline-flex items-center gap-1 text-[10px] text-muted-foreground hover:text-[#A95B61] md:text-[11px]">
            <ChevronRight className="h-3.5 w-3.5" /> {brand.name}
          </Link>
          ...
          <div className="mt-4 flex items-end justify-between gap-4">
            <div className="min-w-0">
              <div className="mb-1.5 flex items-center gap-2">
                ...
                <span className="font-serif text-[7px] uppercase tracking-[0.2em] text-[#B86168]">COLLECTION</span>
              </div>
          ...
          <section className="border-y border-border/60">
            <div className="mx-auto flex h-[54px] w-full max-w-[1400px] items-center justify-between gap-3 px-4 md:px-6">
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-[10px] ${inStockOnly ? "border-[#D6AAA7] bg-[#FFF7F5] text-[#A95B61]" : "border-border text-muted-foreground"}`}
              >
                المتوفر فقط
              </button>
              ...
              <label className="relative flex items-center">
                <select
                  ...
                  className="appearance-none bg-transparent py-2 pl-6 pr-2 text-[10px] font-medium outline-none"
                >
        ...
        <section className="mx-auto mt-5 w-full max-w-[1400px] px-4 md:mt-7 md:px-6">
          {productsLoading ? (
            ...
            <div className="flex min-h-[250px] items-center justify-center border-y border-border/60 text-center">
              <div>
                <Package className="mx-auto h-5 w-5 text-muted-foreground" />
                <h2 className="mt-3 text-[12px] font-semibold">تعذر تحميل المنتجات</h2>
                <p className="mt-1.5 text-[9px] text-muted-foreground">أعد تحميل الصفحة للمحاولة مرة أخرى.</p>
              </div>
            </div>
            ...
            <div className="flex min-h-[250px] items-center justify-center border-y border-border/60 text-center">
              <div>
                <Package className="mx-auto h-5 w-5 text-muted-foreground" />
                <h2 className="mt-3 text-[12px] font-semibold">لا توجد منتجات مطابقة</h2>
                <p className="mt-1.5 text-[9px] text-muted-foreground">لا توجد منتجات في هذا القسم بعد.</p>
              </div>
            </div>
          ) : (
