...
const CategoriesPage = () => {
  ...
  const { data: categories = [], isLoading: categoriesLoading } = useQuery({
    queryKey: ["categories-all-active-v3"],
    queryFn: async () => {
  ...
  const subCategories = useMemo(() => {
    ...
    const audienceSlugs = selectedParent.slug === "women"
      ? WOMEN_CATEGORY_SLUGS
      : selectedParent.slug === "men"
        ? MEN_CATEGORY_SLUGS
        : null;
  ...
  const productScopeKey = useMemo(() => {
    return [audienceContext, audienceRootOnly ? "audience-root" : scopedCategoryIds.join(","), brandFilter, productQuery, productSort, inStockOnly ? "1" : "0", minPrice, maxPrice].join("|");
  }, [audienceContext, audienceRootOnly, scopedCategoryIds, brandFilter, productQuery, productSort, inStockOnly, minPrice, maxPrice]);

  useEffect(() => {
    if (previousScopeRef.current && previousScopeRef.current !== productScopeKey) setLoadedPage(1);
  ...
  const updateProductFilters = (values: ProductListFilterValues) => {
    setStepParams({ brand: values.brand === "all" ? null : values.brand, q: values.query || null, sort: values.sort === "new" ? null : values.sort, stock: values.inStockOnly ? "1" : null, min: values.minPrice || null, max: values.maxPrice || null });
  };
        ...
        {!selectedParent && (
          <section className="border-b border-[#F0E7E3] bg-[#FFF8F6]">
            <div className="mx-auto w-full max-w-[1500px] px-4 pb-5 pt-6 md:px-6 md:pb-8 md:pt-9">
              <div>
                <div className="mb-2 flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#D4777D]" /><span className="font-serif text-[7px] tracking-[0.24em] text-[#B86168]">{getSiteText(content, "categories_page_eyebrow", "FLAMINGO")}</span></div>
          ...
          <section className="border-b border-[#F0E7E3] bg-[#FFFDFC]">
            <div className="mx-auto flex h-[46px] w-full max-w-[1500px] items-center justify-between gap-3 px-3 md:h-[50px] md:px-6">
              <div className="flex min-w-0 items-center gap-1.5 overflow-hidden text-[9px] text-[#A0938E] md:text-[10px]">
                ...
                {selectedSub ? <><button type="button" onClick={() => setStepParams({ sub: null, brand: null })} className="max-w-[100px] truncate text-[#756763] transition-colors hover:text-[#B86168] md:max-w-[180px]">{selectedParent.name_ar}</button><ChevronLeft className="h-3 w-3 shrink-0 stroke-[1.4] text-[#C9BBB6]" /><span className="max-w-[110px] truncate font-semibold text-[#473A38] md:max-w-[200px]">{selectedSub.name_ar}</span></> : <span className="max-w-[150px] truncate font-semibold text-[#473A38]">{selectedParent.name_ar}</span>}
              </div>
              {!!activeProductCategory && availableBrands.length > 0 && <div className="relative shrink-0">{brandOpen && <div className="absolute left-0 top-10 z-50 w-48 overflow-hidden rounded-[14px] border border-[#EDE3DF] bg-white shadow-[0_12px_30px_rgba(60,40,35,.09)]"><div className="max-h-56 overflow-y-auto p-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><button type="button" onClick={() => { setStepParams({ brand: null }); setBrandOpen(false); }} className={`w-full rounded-[10px] px-3 py-2 text-right text-[9px] ${brandFilter === "all" ? "bg-[#FAECE9] font-semibold text-[#B86168]" : "text-[#6B5D59]"}`}>كل الماركات</button>{availableBrands.map((brand) => <button key={brand} type="button" onClick={() => { setStepParams({ brand }); setBrandOpen(false); }} className={`w-full rounded-[10px] px-3 py-2 text-right text-[9px] ${brandFilter === brand ? "bg-[#FAECE9] font-semibold text-[#B86168]" : "text-[#6B5D59]"}`}>{brand}</button>)}</div></div>}</div>}
            </div>
          </section>
        )}
