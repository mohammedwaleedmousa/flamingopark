...
const CategoriesPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDFC] text-[#302725]" dir="rtl"><Navbar /><CartDrawer /><main className="pb-20 md:pt-24">
      ...
      {selectedParent && <section className="border-b border-[#F0E7E3] bg-[#FFFDFC]"><div className="mx-auto flex h-[46px] w-full max-w-[1500px] items-center justify-between gap-3 px-3 md:h-[50px] md:px-6"><div className="flex min-w-0 items-center gap-1.5 overflow-hidden text-[9px] text-[#A0938E] md:text-[10px]"><button type="button" onClick={() => setStepParams({ parent: null, sub: null, brand: null }, { restoreKey: "root" })} className="shrink-0 transition-colors hover:text-[#B86168]">الأقسام</button><ChevronLeft className="h-3 w-3 shrink-0 stroke-[1.4] text-[#C9BBB6]" />{selectedSub ? <><button type="button" onClick={() => setStepParams({ sub: null, brand: null }, { restoreKey: `parent:${selectedParent.slug}` })} className="max-w-[100px] truncate text-[#756763] transition-colors hover:text-[#B86168] md:max-w-[180px]">{selectedParent.name_ar}</button><ChevronLeft className="h-3 w-3 shrink-0 stroke-[1.4] text-[#C9BBB6]" /><span className="max-w-[110px] truncate font-semibold text-[#473A38] md:max-w-[200px]">{selectedSub.name_ar}</span></> : <span className="max-w-[150px] truncate font-semibold text-[#473A38]">{selectedParent.name_ar}</span>}</div>{!!activeProductCategory && availableBrands.length > 0 && <div className="relative shrink-0">{brandOpen && <div className="absolute left-0 top-10 z-50 w-48 overflow-hidden rounded-[14px] border border-[#EDE3DF] bg-white shadow-[0_12px_30px_rgba(60,40,35,.09)]"><div className="max-h-56 overflow-y-auto p-1.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><button type="button" onClick={() => { setStepParams({ brand: null }); setBrandOpen(false); }} className={`w-full rounded-[10px] px-3 py-2 text-right text-[9px] ${brandFilter === "all" ? "bg-[#FAECE9] font-semibold text-[#B86168]" : "text-[#6B5D59]"}`}>كل الماركات</button>{availableBrands.map((brand) => <button key={brand} type="button" onClick={() => { setStepParams({ brand }); setBrandOpen(false); }} className={`w-full rounded-[10px] px-3 py-2 text-right text-[9px] ${brandFilter === brand ? "bg-[#FAECE9] font-semibold text-[#B86168]" : "text-[#6B5D59]"}`}>{brand}</button>)}</div></div>}</div>}</div></section>}
      {!selectedParent && <section className="mx-auto w-full max-w-[1500px] px-2.5 pb-8 pt-3 md:px-6 md:pb-12 md:pt-6">{categoriesLoading ? <div className="grid grid-cols-3 gap-2 md:gap-5">{Array.from({ length: 6 }).map((_, index) => <div key={index} className="aspect-[4/5] animate-pulse rounded-[14px] bg-[#F2ECE9] md:rounded-[17px]" />)}</div> : <div className="grid grid-cols-3 gap-2 md:gap-5">{parents.map((category, index) => <Link key={category.id} to={`/categories?parent=${category.slug}`} onClick={() => rememberScroll("root")} className="group relative aspect-[4/5] overflow-hidden rounded-[14px] bg-[#F4F1EF] md:rounded-[20px]"><img src={optimizeImage(category.image_url || FALLBACK[category.slug] || FALLBACK.women, 960, 78)} srcSet={createImageSrcSet(category.image_url || FALLBACK[category.slug] || FALLBACK.women, [240, 480, 960], 78)} sizes="(max-width: 767px) 33vw, 33vw" alt={category.name_ar} loading={index < 3 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" width={960} height={1200} onError={handleImageError} className="h-full w-full object-cover md:transition-transform md:duration-300 md:group-hover:scale-[1.02]" /><div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" /><div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-1 p-2 md:gap-2 md:p-4"><div className="min-w-0">{category.name && <p className="mb-0.5 truncate text-[5px] tracking-[0.08em] text-white/65 md:text-[8px] md:tracking-[0.11em]">{category.name}</p>}<h2 className="truncate text-[12px] font-semibold text-white sm:text-[13px] md:text-[22px]">{category.name_ar}</h2></div><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white text-[#C96F79] md:h-8 md:w-8"><ChevronLeft className="h-3 w-3 stroke-[1.7] md:h-3.5 md:w-3.5" /></span></div></Link>)}</div>}</section>}
      {selectedParent && !selectedSub && subCategories.length > 0 && <><section className="mx-auto w-full max-w-[1500px] px-4 pb-3 pt-5 md:px-6 md:pb-5 md:pt-7"><div className="flex items-end justify-between"><div><div className="mb-1.5 flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#D4777D]" /><span className="font-serif text-[6px] tracking-[0.22em] text-[#B86168]">FLAMINGO</span></div><h1 className="text-[22px] font-semibold tracking-[-0.035em] text-[#403132] md:text-[30px]">{selectedParent.name_ar}</h1><p className="mt-1 text-[8px] text-[#9C8D88] md:text-[10px]">اختاري المجموعة التي تريدين استكشافها</p></div><span className="text-[8px] font-medium text-[#B96B70]">{subCategories.length} أقسام</span></div></section><section className="mx-auto w-full max-w-[1500px] px-2.5 pb-9 md:px-6 md:pb-12"><div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 md:grid-cols-4 md:gap-5">{subCategories.map((category, index) => <Link key={category.id} to={`/categories?parent=${selectedParent.slug}&sub=${category.slug}`} onClick={() => rememberScroll(`parent:${selectedParent.slug}`)} className="group overflow-hidden rounded-[16px] border border-[#EEE5E1] bg-white"><div className="aspect-square overflow-hidden bg-[#F4F1EF]"><img src={optimizeImage(category.image_url || FALLBACK[category.slug] || FALLBACK.women, 640, 78)} srcSet={createImageSrcSet(category.image_url || FALLBACK[category.slug] || FALLBACK.women, [240, 400, 640], 78)} sizes="(max-width: 639px) 50vw, (max-width: 767px) 33vw, 25vw" alt={category.name_ar} loading={index < 2 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" width={640} height={640} onError={handleImageError} className="h-full w-full object-cover md:transition-transform md:duration-300 md:group-hover:scale-[1.02]" /></div><div className="flex min-h-[52px] items-center justify-between gap-2 px-3 py-2.5"><div className="min-w-0"><h2 className="truncate text-[11px] font-semibold text-[#453937] md:text-[13px]">{category.name_ar}</h2>{category.name && <p className="mt-0.5 truncate text-[7px] text-[#A0938E] md:text-[8px]">{category.name}</p>}</div><ChevronLeft className="h-3.5 w-3.5 shrink-0 stroke-[1.5] text-[#C96F79]" /></div></Link>)}</div></section></>}
