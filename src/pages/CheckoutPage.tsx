...
const CheckoutPage = () => {
  ...
  const [deliveryScope, setDeliveryScope] = useState<"aden" | "outside">("aden");
  ...
  const flamingoDeliveryCompany = useMemo(() => deliveryCompanies.find((company) => company.name.trim().includes("فلامنجو")) || null, [deliveryCompanies]);
  const visibleDeliveryCompanies = useMemo(() => deliveryScope === "aden" ? (flamingoDeliveryCompany ? [flamingoDeliveryCompany] : []) : deliveryCompanies.filter((company) => company.id !== flamingoDeliveryCompany?.id), [deliveryCompanies, deliveryScope, flamingoDeliveryCompany]);

  useEffect(() => {
    if (deliveryScope === "aden") {
      setSelectedDelivery(flamingoDeliveryCompany?.id || "");
      return;
    }

    if (selectedDelivery === flamingoDeliveryCompany?.id) setSelectedDelivery("");
  }, [deliveryScope, flamingoDeliveryCompany?.id]);
  ...
  const selectSavedAddress = (id: string) => {
    ...
    if (!address) return;
    setFormData((current) => ({ ...current, name: isGuestLike ? String(address.name || current.name || "") : current.name, phone: isGuestLike ? String(address.phone || current.phone || "") : current.phone, city: address.city, address: address.address, notes: address.notes || "" }));
  };
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 1 && (!formData.city.trim() || !formData.address.trim())) { toast({ title: "المدينة والعنوان مطلوبان", variant: "destructive" }); return false; }
    if (step === 2) {
      ...
      if (isCashPayment && codRegions.length > 0 && !selectedRegion) { toast({ title: "اختر منطقة الاستلام", variant: "destructive" }); return false; }
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const orderData = { orderId: createdOrder.order_id, orderNumber: createdOrder.order_number, trackingToken: createdOrder.tracking_token, customerName: customerName || "عميل", customerPhone, customerAddress: formData.address || "-", customerCity: formData.city || "", customerNotes: formData.notes || "", items: validation.data, subtotal: Number(createdOrder.subtotal), deliveryFee: Number(createdOrder.delivery_fee), discountAmount: Number(createdOrder.discount_amount), couponCode: Number(createdOrder.discount_amount) > 0 ? appliedCoupon : null, total: Number(createdOrder.total), paymentMethod, paymentMethodName: selectedPaymentMethod?.name_ar || selectedPaymentMethod?.name || paymentMethod, deliveryCompany: createdOrder.delivery_company || selectedCompany?.name || "", selectedRegion: isCashPayment && regionData ? regionData.region_name_ar : customer?.region || null, country: orderCountryFromPhone(customerPhone), currencyMode: createdOrder.currency_mode || currencyMode, createdAt: createdOrder.created_at || new Date().toISOString() };
      clearCart();
      navigate("/order-confirmation", { state: { orderData } });
    } catch (error) {
      const rawMessage = error instanceof Error ? error.message : "فشل إرسال الطلب";
      const message = rawMessage.includes("invalid_checkout_phone") || rawMessage.includes("invalid_yemen_phone") ? "أدخل رقم هاتف صحيح. لليمن استخدم 9 أرقام تبدأ بـ7، وللدول الأخرى استخدم رمز الدولة." : rawMessage.includes("order_rate_limit") ? "تم إرسال عدة طلبات خلال فترة قصيرة. حاول مرة أخرى بعد قليل." : rawMessage.includes("guest_order_capacity_limit") ? "إنشاء الطلبات غير متاح مؤقتاً بسبب عدد كبير من المحاولات. حاول بعد قليل." : rawMessage.includes("insufficient_stock") ? "تغير مخزون أحد المنتجات في السلة. راجع الكمية وحاول مرة أخرى." : rawMessage.includes("variant_selection_required") ? "تحقق من اختيار المقاس أو اللون لجميع المنتجات." : rawMessage.includes("invalid_coupon") ? "كود الخصم لم يعد صالحاً. أزله أو جرّب كوداً آخر." : rawMessage.includes("invalid_payment_method") ? "طريقة الدفع المختارة غير متاحة حالياً. اختر طريقة دفع أخرى." : rawMessage.includes("invalid_delivery_company") || rawMessage.includes("delivery_company_required") ? "شركة التوصيل المختارة غير متاحة حالياً. اختر شركة توصيل أخرى." : rawMessage;
