...
const CheckoutPage = () => {
  ...
  const [couponDiscount, setCouponDiscount] = useState<{ type: "percentage" | "fixed"; value: number } | null>(null);
  ...
  const { total, discountAmount } = calculateOrderTotals(
    subtotal,
    deliveryFee,
    appliedCoupon && couponDiscount ? couponDiscount.value : launchDiscountEligible ? 10 : 0,
    appliedCoupon && couponDiscount ? couponDiscount.type : "percentage",
  );
  ...
  const applyCoupon = async () => {
    ...
    try {
      ...
      const type = data.type === "percentage" ? "percentage" : "fixed";
      const { discountAmount: discount } = calculateOrderTotals(subtotal, deliveryFee, value, type);
      setCouponDiscount({ type, value }); setAppliedCoupon(normalized);
  ...
  const removeCoupon = () => { setCouponCode(""); setAppliedCoupon(null); setCouponDiscount(null); toast({ title: "تمت إزالة الكوبون" }); };
