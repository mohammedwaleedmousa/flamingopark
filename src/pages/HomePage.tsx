...
import DynamicSection, { type HomepageSection } from "@/components/DynamicSection";
  ...
  <section className="w-full overflow-hidden bg-background py-4 md:py-6" dir="rtl" aria-label="الأقسام">
    <div className="mx-auto w-full max-w-[1400px] px-3 md:px-6">
      <div className="mb-3 flex items-end justify-between gap-3 md:mb-4">
        <div>
          <div className="mb-1 flex items-center gap-2"><span className="h-[2px] w-4 rounded-full bg-[#D4777D]" /><span className="font-serif text-[6px] uppercase tracking-[0.2em] text-[#B86168]">CATEGORIES</span></div>
        ...
        {items.map((item) => (
          <SwiperSlide key={`${item.title}-${item.link}`} className="!w-[78px] sm:!w-[90px] md:!w-[102px]">
            <Link to={item.link} className="group block w-full select-none [-webkit-tap-highlight-color:transparent]">
              <div className="aspect-square w-full overflow-hidden rounded-[15px] border border-border/60 bg-muted/40 md:rounded-[18px]"><img src={item.image} alt={item.title} loading="lazy" decoding="async" className="h-full w-full object-cover object-center transition-transform duration-300 group-hover:scale-[1.025]" /></div>
              <div className="mt-1.5 text-center"><p className="truncate text-[8px] font-semibold text-foreground md:text-[9px]">{item.title}</p><p className="mt-0.5 truncate font-serif text-[5px] uppercase tracking-[0.08em] text-muted-foreground md:text-[6px]">{item.subtitle}</p></div>
            </Link>
          </SwiperSlide>
        ))}
      </Swiper>
    ...
    <section className="bg-background py-7 md:py-12">
      <div className="mx-auto w-full max-w-[1400px] px-3 md:px-6">
        <div className="mb-4 flex items-end justify-between gap-3 md:mb-7">
          <div className="min-w-0"><div className="mb-1 flex items-center gap-2"><span className="h-[2px] w-4 shrink-0 rounded-full bg-[#D4777D]" /><span className="truncate font-serif text-[6px] uppercase tracking-[0.2em] text-[#B86168] md:text-[7px]">{eyebrow}</span></div><h2 className="text-[17px] font-semibold tracking-[-0.025em] text-foreground md:text-[26px]">{title}</h2></div>
          <Link to={link} className="flex shrink-0 items-center gap-1 border-b border-border pb-0.5 text-[7px] font-medium text-[#A95B61] transition-opacity active:opacity-60 md:text-[8px]">عرض الكل<ArrowLeft className="h-3 w-3" strokeWidth={1.5} /></Link>
        </div>
        <div className="grid grid-cols-2 gap-x-2.5 gap-y-5 sm:gap-x-3 md:grid-cols-4 md:gap-x-5 md:gap-y-7">{products.slice(0, 8).map((product) => <ProductCard key={product.id} product={product} badge={badge} />)}</div>
      </div>
    </section>
  );
};
...
const EditorialSection = () => (
  <section className="bg-background px-4 py-11 md:py-20">
    <div className="mx-auto max-w-[850px] text-center">
      ...
      <h2 className="mx-auto max-w-[700px] text-[21px] font-light leading-[1.8] tracking-[-0.025em] text-foreground md:text-[36px] md:leading-[1.7]">الأناقة ليست ما ترتديه،<br />بل ما يبقى في الذاكرة.</h2>
      ...
      <Link to="/products" className="mx-auto mt-5 inline-flex items-center gap-1.5 border-b border-border pb-1 text-[7px] font-semibold text-[#A95B61] md:text-[8px]">اكتشف المجموعة<ArrowLeft className="h-3 w-3" strokeWidth={1.5} /></Link>
    </div>
  </section>
);
...
const HomePage = () => {
  ...
  const featuredCategories = useMemo<FeaturedCategoryItem[]>(() => {
    ...
    return parentCategories.map((category: any) => ({ title: category.name_ar || category.name || category.slug, subtitle: category.name || category.name_ar || category.slug, image: category.image_url || fallbackCategoryImages[category.slug] || fallbackFeaturedCategories[0].image, link: `/categories?parent=${category.slug}` }));
  ...
  const { data: dynamicSections = [] } = useQuery({
    queryKey: ["home-dynamic-sections"],
    enabled: showHomeSection("dynamicSections"),
    queryFn: async () => {
      const { data, error } = await supabase.from("homepage_sections").select("id,title,title_ar,filter_type,max_products,show_view_all,view_all_link").eq("is_active", true).order("sort_order", { ascending: true });
      if (error) throw error;
  ...
  return (
    <div className="relative min-h-screen bg-background" dir="rtl">
      ...
      <main className="overflow-hidden bg-background">
        ...
        {showHomeSection("brands") && <div ref={brandsViewport.ref} className="bg-background" style={{ minHeight: 92 }}><BrandsStrip enabled={brandsViewport.isNearViewport} /></div>}
        {showHomeSection("dynamicSections") && dynamicSections.map((section, index) => <DynamicSection key={section.id} section={section} country="GLOBAL" index={index} />)}
        {showHomeSection("featuredProducts") && <div ref={featuredViewport.ref} className="min-h-px bg-background"><ProductSection eyebrow="FLAMINGO PICKS" title="مختارات فلامنجو" products={products} link="/curated" /></div>}
        {showHomeSection("services") && <div className="bg-background"><FlamingoServices /></div>}
        {showHomeSection("bestSellers") && <div ref={bestSellersViewport.ref} className="min-h-px bg-background"><ProductSection eyebrow="BEST SELLERS" title="الأكثر مبيعاً" products={bestSellers} link="/top-selling" badge="BEST SELLER" /></div>}
