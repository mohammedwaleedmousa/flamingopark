...
const NewArrivalsPage = () => {
  ...
  const { data: products = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("products").select(PRODUCT_CARD_SELECT).eq("is_active", true).order("created_at", { ascending: false }).limit(36);
            ...
            <div className="flex items-end justify-between gap-5">
              <div>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="font-serif text-[7px] tracking-[0.25em] text-[#B75F66]">
                    {getSiteText(content, "new_arrivals_eyebrow", "NEW IN")}
                  </span>
                </div>
              ...
              {!isLoading && products.length > 0 && (
                <div className="shrink-0 text-left">
                  ...
                  <span className="mt-1 block text-[6px] text-[#A99A94] md:text-[7px]">قطعة جديدة</span>
                </div>
              )}
            </div>
          </div>
        ...
        <section className="border-b border-[#EFE6E2] bg-white">
          <div className="mx-auto flex h-[42px] w-full max-w-[1500px] items-center justify-between px-3 md:h-[46px] md:px-6">
            <div className="flex items-center gap-1.5">
              <Sparkles className="h-3.5 w-3.5 stroke-[1.5] text-[#C96F79]" />
              <span className="text-[8px] font-medium text-[#755F5E] md:text-[9px]">أحدث الإضافات</span>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex min-h-[48vh] flex-col items-center justify-center px-5 text-center">
              <div className="flex h-[64px] w-[64px] items-center justify-center rounded-full bg-[#FAECE9]">
                <Sparkles className="h-5 w-5 stroke-[1.4] text-[#C76D73]" />
              </div>
            ...
            <div className="grid grid-cols-2 gap-x-2.5 gap-y-5 sm:gap-x-3 sm:gap-y-6 md:grid-cols-3 md:gap-x-5 md:gap-y-8 lg:grid-cols-4 xl:grid-cols-5">
              {products.map((product) => (
                <div key={product.id} className="min-w-0">
                  <ProductCard product={product} badge="NEW IN" />
                </div>
              ))}
            </div>
          )}
          ...
          <section className="mx-auto w-full max-w-[1500px] px-3 py-9 md:px-6 md:py-12">
            <div className="border-t border-[#EADFDA] pt-6 text-center">
              <div className="mb-2 flex items-center justify-center gap-2">
                ...
                <span className="font-serif text-[6px] tracking-[0.24em] text-[#B86168]">FLAMINGO NEW EDIT</span>
                <span className="h-px w-5 bg-[#D9B6B2]" />
              </div>

              <p className="text-[9px] text-[#968783]">تتجدد اختياراتنا باستمرار لتكتشف كل جديد.</p>
            </div>
          </section>
        )}
      </main>
