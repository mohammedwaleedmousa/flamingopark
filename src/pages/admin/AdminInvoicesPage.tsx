...
const AdminInvoicesPage = () => {
  ...
  const { data: orders = [], isLoading: isLoadingOrders, isFetching: isFetchingOrders } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("orders").select("id,order_number,customer_name,customer_phone,customer_address,customer_notes,items,subtotal,delivery_fee,total,total_base,payment_method,country,created_at,status,invoice_url,coupon_code,discount_amount,currency_code,currency_mode,exchange_rate_snapshot,sales_agent_id,sales_agent_code,sales_agent_name,invoice_review_status,invoice_reviewed_at,invoice_reviewed_by,invoice_review_note,delivery_companies(name)").order("created_at", { ascending: false });
  ...
  const { data: salesAgents = [], isLoading: isLoadingSalesAgents } = useQuery({
    queryKey: ["invoice-sales-agents"],
    queryFn: async () => {
  ...
  const assignSalesAgentMutation = useMutation({
    mutationFn: async ({ orderId, agentId }: { orderId: string; agentId: string | null }) => {
      const agent = agentId ? salesAgents.find((item) => item.id === agentId) : null;
      if (agentId && !agent) throw new Error("تعذر العثور على موظف المبيعات المحدد.");

      const { error } = await (supabase as any)
        .from("orders")
        .update({
          sales_agent_id: agent?.id || null,
          sales_agent_code: agent?.code || null,
          sales_agent_name: agent?.name || null,
        })
        .eq("id", orderId);
      if (error) throw error;
    },
    onSuccess: async (_data, variables) => {
      const agent = variables.agentId ? salesAgents.find((item) => item.id === variables.agentId) : null;
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ["electronic-sales-agent-details"] }),
      ]);
      toast({
        title: agent ? `تم إسناد الفاتورة إلى ${agent.name}` : "تم إلغاء نسبة الفاتورة لموظف",
        description: "تم تحديث صفحة المبيعات الإلكترونية تلقائيًا.",
      });
    },
    onError: (error: any) => toast({
      title: "تعذر تحديث موظف المبيعات",
      description: error?.message || "حدث خطأ أثناء حفظ النسبة.",
      variant: "destructive",
    }),
  });
...
const OrdersPanel = ({ orders, loading, fetching, invoiceOrderNumberSet, activeTab, selectedIds, allVisibleSelected, salesAgents, salesAgentsLoading, assigningOrderId, onAssignSalesAgent, onToggleAll, onToggleSelected, onOpenEditor, onAccept, onReject, onReturn, onReturnToReview, reviewBusy }: { orders: Order[]; loading: boolean; fetching: boolean; invoiceOrderNumberSet: Set<string>; activeTab: TabMode; selectedIds: string[]; allVisibleSelected: boolean; salesAgents: SalesAgent[]; salesAgentsLoading: boolean; assigningOrderId: string | null; onAssignSalesAgent: (order: Order, agentId: string | null) => void; onToggleAll: () => void; onToggleSelected: (id: string) => void; onOpenEditor: (order: Order) => void; onAccept: (order: Order) => void; onReject: (order: Order) => void; onReturn: (order: Order) => void; onReturnToReview: (order: Order) => void; reviewBusy: boolean }) => {
