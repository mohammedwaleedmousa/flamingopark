...
const AdminProductClassificationPage = () => {
  ...
  const [revisions, setRevisions] = useState<Record<string, ProductRevision[]>>({});
  ...
  const visibleRows = useMemo(() => {
    ...
    if (!q) return rows;
    return rows.filter((row) => [row.name, row.name_ar, row.slug, row.brand, row.category]
  ...
  const openHistory = async (productId: string, force = false) => {
    ...
    try {
      const next = await listProductRevisions(productId, 20);
      setRevisions((current) => ({ ...current, [productId]: next }));
    } catch (error) {
      console.error(error);
  ...
  const apply = async (row: ProductClassificationRow, field: "brand" | "category" | "audience") => {
    const suggestion = field === "brand"
      ? row.brandSuggestion
      : field === "category"
        ? row.categorySuggestion
        : row.audienceSuggestion;
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "راجع المنتج";
      toast({ title: "تعذر تطبيق الاقتراح", description: message, variant: "destructive" });
    } finally {
      setBusyId(null);
    }
  };

  const undo = async (productId: string, revision: ProductRevision) => {
    setBusyId(`undo:${revision.id}`);
    try {
      await undoProductRevision(revision.id);
      toast({ title: "تم التراجع بأمان", description: "تم حفظ التراجع كسجل جديد بدون لمس المخزون." });
      await load();
      await openHistory(productId, true);
    } catch (error) {
      const message = error instanceof Error ? error.message : "تعذر التراجع";
      toast({
        title: "لم يتم التراجع",
        description: message.toLowerCase().includes("stale")
          ? "المنتج تغيّر بعد هذا التعديل، لذلك تم منع التراجع لحماية التعديل الأحدث."
          : message,
        variant: "destructive",
      });
    } finally {
...
const audienceLabel = (value: string | null | undefined) => ({
  men: "رجالي",
  women: "نسائي",
  kids: "أطفال",
  unisex: "للجميع",
}[String(value || "")] || "بدون جمهور");

const MetaPill = ({ value, muted = false }: { value: string; muted?: boolean }) => (
  <span className={cn(
    "rounded-full border px-[7px] py-[4px] text-[6.5px]",
    muted ? "border-[#ECEFF2] bg-[#F7F8F9] text-[#A0A7AF]" : "border-[#E3E7EB] bg-white text-[#747C86]",
  )}>{value}</span>
);
