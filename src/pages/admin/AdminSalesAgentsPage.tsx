...
const AdminSalesAgentsPage = () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      toast({ title: wasEditing ? "تم تحديث الموظف" : "تمت إضافة الموظف" });
    },
  ...
  return (
    <div className="w-full space-y-4" dir="rtl">
      <AdminPageHeader category="المبيعات" title="أكواد الموظفين" description="إنشاء كود خاص لكل موظف وربط الطلبات والمبيعات به تلقائيًا" actions={[{ label: "موظف جديد", icon: Plus, onClick: openCreate, variant: "primary" }]} />
...
const Stat = ({ title, value, helper, icon }: { title: string; value: string; helper: string; icon: ReactNode }) => (
