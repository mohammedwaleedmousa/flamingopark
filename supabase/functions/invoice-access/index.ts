...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: order, error } = await service
      .from('orders')
      .select('id,order_number,invoice_url,owner_user_id,guest_access_token_hash')
      .eq('id', orderId)
      .maybeSingle()
    ...
    if (action === 'upload') {
      // An owner can read an invoice but cannot replace it.
      if (!isAdmin && !validGuestToken) return new Response(JSON.stringify({ error: 'Invoice access denied' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    ...
    if (!isAdmin && !isOwner && !validGuestToken) return new Response(JSON.stringify({ error: 'Invoice access denied' }), { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
