-- Connect checkout payment validation to the payment methods managed by the admin panel.
-- Existing orders are untouched. Only active payment methods can be used for new orders.
...
begin
  ...
  -- Production may already contain this patch. Keep the migration idempotent so
  -- future deploys do not fail merely because the function is already connected.
  if position('public.payment_methods' in ddl) > 0 and position('p_payment_method not in (''cod'',''bank'')' in ddl) = 0 then
    return;
  end if;
