-- Fix catalog health image detection for products whose approved images live in color_variants[].images.
-- Existing product data is not modified.
  ...
  select jsonb_build_object(
    ...
    'products_with_issues', count(*) filter (where
      (
        ...
        and not exists (
          select 1
          from jsonb_array_elements(coalesce(color_variants, '[]'::jsonb)) variant
          where jsonb_typeof(variant) = 'object'
            and jsonb_typeof(variant->'images') = 'array'
            and jsonb_array_length(variant->'images') > 0
        )
